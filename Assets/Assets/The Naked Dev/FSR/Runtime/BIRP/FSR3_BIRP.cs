...
namespace TND.FSR
{
    ...
    public class FSR3_BIRP : FSR3_BASE
    {
        ...
        private void SetupCommandBuffer() {
            ...
            m_dispatchDescription.UpscaleSize = new Vector2Int(m_displayWidth, m_displayHeight);
            m_dispatchDescription.Color = new ResourceView(m_colorBuffer);
            if (m_mainCamera.actualRenderingPath == RenderingPath.Forward)
            {
                m_dispatchDescription.Depth = new ResourceView(BuiltinRenderTextureType.Depth);
            }
            else
            {
                m_dispatchDescription.Depth = new ResourceView(BuiltinRenderTextureType.ResolvedDepth);
            }
            m_dispatchDescription.MotionVectors = new ResourceView(BuiltinRenderTextureType.MotionVectors);
            ...
            if (generateReactiveMask) {
                ...
                m_dispatchDescription.Reactive = new ResourceView(m_reactiveMaskOutput);
            } else {
            ...
            if (generateTCMask)
            {
                if (generateReactiveMask)
                {
                    m_dispatchDescription.ColorOpaqueOnly = new ResourceView(m_reactiveMaskOutput);
                }
                else
                {
                    m_dispatchDescription.ColorOpaqueOnly = new ResourceView(m_opaqueOnlyColorBuffer);
                }
            }
            else
            {
                m_dispatchDescription.ColorOpaqueOnly = ResourceView.Unassigned;
            }
